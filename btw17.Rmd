...
```{r}
# get map & add turnout
ger <- readRDS(url("http://biogeo.ucdavis.edu/data/gadm2.8/rds/DEU_adm1.rds"))
ger$Prozent <- turnout$Prozent

# create labels and color palette
labels <- paste0(
  ger$NAME_1, ": ", round(ger$Prozent * 100, 1), "%"
) %>% lapply(htmltools::HTML)

pal <- colorBin("YlGnBu", domain = turnout$Prozent)
...
leaflet(ger) %>%
  ...
  addPolygons(weight = 2, color = "black", fillColor = ~pal(Prozent),
              ...
              highlight = highlightOptions(weight = 3, color = "#800000",
                                           ...
                                           bringToFront = TRUE)) %>%
  addLegend(pal = pal, values = ~Prozent, opacity = 0.7, title = NULL,
